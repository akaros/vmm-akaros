...
fn boot_linux() {
    ...
    let low_mem_size = Some(4 * GiB);
    let memory_size = 4 * GiB;
    let vmm = VMManager::new().unwrap();
    let kn_path = env::var("FIRMWARE_PATH").unwrap();
    ...
    let vm = Arc::new(vm);
    // We need to be able to load an arbitrary image here.
    // At an arbitrary location. And then we want to run it with full tracing.
